    ...
    for super_node in network.super_qkd_nodes.values():
        ...
        for tr in super_node.transceivers.values():
            ...
            for c in tr.qkd_node.cchannels.values():
                n = re.search("cchannel(.+)_", c.name).group(1)  # type: ignore
                cc[n] = 0

        row_4.append(tot_ric)  # type: ignore
...
def main():
    ...
    parser.add_argument(
        "--netx-graph", dest="netx_graph", type=str, help="File json for networkX graph"
    )
    parser.add_argument(
        "--seq-graph", dest="seq_graph", type=str, help="File json for sequence graph"
    )
    parser.add_argument(
        "--key-size", dest="key_size", type=int, default=128, help="Key size in bits"
    )
    parser.add_argument(
        "--mess-rate",
        dest="mess_rate",
        type=float,
        default=0.1,
        help="Message sending rate (exponential dist.)",
    )
    parser.add_argument(
        "--num-nodes",
        dest="num_nodes",
    ...
    parser.add_argument(
        ...
        type=float,
        default=10,
        help="Buffer capacity of the nodes",
    )
    parser.add_argument(
        "--inspection-rate",
        dest="inspection_rate",
        type=float,
        default=0.005,
        help="Inspection rate",
    )
    parser.add_argument(
        "--traffic",
        dest="traffic",
    ...
    MessagingProtocol.sim_command = sim_command  # type: ignore
    QKDTopoExt.sim_command = sim_command  # type: ignore
    ...
    elif (args.netx_graph != None and args.seq_graph != None) or (
        args.netx_graph == None and args.seq_graph != None
    ):
