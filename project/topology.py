...
from networkx import DiGraph, Graph, exception, shortest_path
...
class QKDTopoExt(Topology):
        ...
        for super_node_name in self.super_qkd_nodes.keys():
            for qc in topo_config.get(self.ALL_Q_CHANNEL):
                ...
                if super_node_name == src_node_name:
                    ...
                    tr_pairs[src_tr_name] = dst_tr_name
                    ...
                    ls_arg = {
                        "LightSource": {"frequency": qc_bit_rate, "mean_photon_num": 1}
                    }
                    ...
                    self.super_qkd_nodes[src_node_name].transceivers[
                        src_tr_name
                    ] = tr_node
    ...
    def add_channels(self, topo_config):
        with open(QKDTopoExt.sim_path + "ber_dist.csv", "w") as file:  # type: ignore
            ...
            for tr in super_node.transceivers.values():
                ...
                for qc in topo_config.get(self.ALL_Q_CHANNEL):
                    if qc[self.SRC] == src_node_name and qc[self.DST] == dst_node_name:
                        ...
                        row = [
                            QKDTopoExt.sim_command,
                            qc_name,
                            str(qc_distance),
                            str(BER),
                        ]

                        with open(QKDTopoExt.sim_path + "ber_dist.csv", "a") as file:  # type: ignore
                            writer = csv.writer(file)
                            writer.writerow(row)
    ...
    def start_pairing(self):
        for super_node in self.super_qkd_nodes.values():
            for tr in super_node.transceivers.values():
                ...
                if (
                    tr.qkd_node.protocol_stack[0].role == -1
                    or tr.qkd_node.protocol_stack[1].role == -1
                ):

                    sender_tr_name = tr.qkd_node.name
                    sender_node_name = super_node.name

                    recv_node_name = re.findall("tr_(.*)_to_(.*)", tr.qkd_node.name)[0][
                        1
                    ]
                    recv_tr_name = "tr_" + recv_node_name + "_to_" + sender_node_name

                    recv_node = (
                        self.super_qkd_nodes[recv_node_name]
                        .transceivers[recv_tr_name]
                        .qkd_node
                    )

                    pair_bb84_protocols(
                        tr.qkd_node.protocol_stack[0], recv_node.protocol_stack[0]
                    )
                    pair_cascade_protocols(
                        tr.qkd_node.protocol_stack[1], recv_node.protocol_stack[1]
                    )
    ...
    def start_qkd(self):
        for super_node in self.super_qkd_nodes.values():
            for tr in super_node.transceivers.values():
                if (
                    tr.qkd_node.protocol_stack[0].role == 0
                    and tr.qkd_node.protocol_stack[1].role == 0
                ):
