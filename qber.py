...
class FSOQKD:
    ...
    def get_rate(self, d, tx_rate):
        """distance in m"""
        self.responsivity = (
            self.eta_q * (self.wave_lambda * 1e6) / 1.23985
        )  # FIXME make this number a parameter
        ...
        self.teta = math.sqrt(
            self.teta_v**2 + self.teta_h**2
        )  # TODO muovi nel corpo della funzione
        ...
        K1 = (
            ...
            * self.eta_d
            * (LA / d**2)
            * (self.wave_lambda / (4 * math.pi)) ** 2
        )
        P_rec = K1 * self.d_gain * self.l_teta
        P_rec2 = 0
        # Not used (I left it here just if we need some additional statistic)
        P_rec = P_rec * (10 ** (1.5 / 10) + 10 ** (3 / 10))
        # Confirmed for filtering (3dB) and housing (1.5dB) from refs. 10 and 11 respectively
        totAtt = P_rec / self.tx_power
        # It's valid... between 0 and 1 (0---->1)
        ...
        if totAtt > 1:
            ...
            print(
                "Warning: detected amplification phenomenon at distance: ",
                d,
                " (m), please check link distance and/or telescope"
                "diameters for the gains",
            )
            # In order to avoid errors due to wrong transmission power setting
            totAtt = 0.999
            # like no atmospheric attenuation
        # I0dB=-20
        # Ranges from -30dB to -10dB
        I0 = P_rec / (math.pi * (self.td_diameter / 2) ** 2)
        # Irradiance (Watts over receiver surface)
        R = self.responsivity
        ...
        # Temperature in °K
        RL = 50
        # Ohm, Load resistance on the receiver unit
        ...
        if (ps - pdark) > 0:
            SK = keyrate0
        return P_rec, SNR, BER, SK, alpha_dB, P_rec2, lambdaQKD, ps, totAtt
