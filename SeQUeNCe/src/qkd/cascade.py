...
class Cascade(StackProtocol):
    ...
    def __init__(self, own: "QKDNode", name: str, w=4, role=-1, secure_params=100):
        ...
        #provvisorio per analisi
        self.received_bit = 0
        self.disclosed_bits_counter_aux = 0
    ...
    def pop(self, info: int) -> None:
        ...
        if self.role == 1 and self.state == 1:
            self.received_bit += info.bit_length()
            #print("Role :",self.role," ", info.bit_length())
    ...
    def received_message(self, src: str, msg: "Message") -> None:
        ...
        if msg.msg_type is CascadeMsgType.KEY:
            ...
            if p == 0:
                p = 0.0001
            self.k1 = get_k1(p, 0, 10000) #Stima la dimensione del blocco al passo 1
            self.state = 1
            ...
            def flip_bit_at_pos(val, pos):
                ...
                self.disclosed_bits_counter_aux += 1
                self.another.disclosed_bits_counter_aux += 1
            ...
            #######
            self.bits[key_id] = self.bits[key_id] >> self.another.disclosed_bits_counter_aux
            self.disclosed_bits_counter_aux = 0
            self.another.disclosed_bits_counter_aux = 0
            #######
            
            for i in range(int(self.bits[key_id].bit_length() / self.keylen)):
    ...
    def check_checksum(self, key_id: int) -> bool:
        ...
        ##########################
        self.bits[key_id] = self.bits[key_id] >> self.disclosed_bits_counter_aux
        #############################
